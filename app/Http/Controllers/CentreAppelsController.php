...
class CentreAppelsController extends Controller
{
    //
    ...
    public function index(){
        $service = ServiceCentre::where('id',2)->first();
        return view('cms.centreDappels',compact('service'));
    }
    ...
    if($request->hasFile('image')){
       $photo = $request->file('image');
         ...
         ServiceCentre::where('id',2)->update([
            'image'=>$image,
         ]);
    ...
    if($request->hasFile('video1')){
       $photo = $request->file('video1');
         ...
         ServiceCentre::where('id',2)->update([  'video1'=>$video1,]);
     }
    ...
    if($request->hasFile('video')){
       $photo = $request->file('video');
         ...
         ServiceCentre::where('id',2)->update([  'video'=>$video,]);
     }
     ...
     return  redirect()->route('centreDappelsCms');
