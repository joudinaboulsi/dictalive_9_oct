...
class ServiceCentreController extends Controller
{
    ...
    public function index(){
        $service_frappe = ServiceCentre::where('id',1)->first();
        return view('cms.centreDeFrappe',compact('service_frappe'));
    }
    ...
    if($request->hasFile('image')){
       $photo = $request->file('image');
         ...
         ServiceCentre::where('id',1)->update([
            'image'=>$image,
         ]);
    ...
    if($request->hasFile('video1')){
       $photo = $request->file('video1');
         ...
         ServiceCentre::where('id',1)->update([  'video1'=>$video1,]);
     }
    ...
    if($request->hasFile('video')){
       $photo = $request->file('video');
         ...
         ServiceCentre::where('id',1)->update([  'video'=>$video,]);
     }
     ...
     return  redirect()->route('centreDeFrappeCms');
